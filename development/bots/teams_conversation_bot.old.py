...
from netbox import get_devices_info, get_rack_list,get_provider_list 

ADAPTIVECARDTEMPLATE = "development/resources/UserMentionCardTemplate.json" 
ADAPTIVECARDTEST = "development/resources/MindBotgreeting.json"
ADAPTIVECARD2 = "development/resources/secondCard.json"
ADAPTIVECARD3 = "development/resources/Greeting.json"

OPTION1_CARD = "development/resources/Option1Card.json"
...
class TeamsConversationBot(TeamsActivityHandler):
    ...
    async def on_message_activity(self, turn_context: TurnContext):
        ...
        if "netbox" in text:
            await self._get_netbox(turn_context)
            return
        
        if "mindbot" in text.lower():
            await self._second_card(turn_context)
            return
        
        if "option-1" in text:
            self.option1 = True
            await self._return_netbox(turn_context)
            return
        ...
        if "option-A1" and self.option1:
            await self._optionA1(turn_context)
            return
        
        if "option1" and self.option1:
            self.option1 = True
            await self._option1_card(turn_context)
            return
    ...
    async def _mention_adaptive_card_activity(self, turn_context: TurnContext):
        ...
        mindbot = os.path.join(os.getcwd(), ADAPTIVECARDTEST)
    ...
    async def _second_card(self, turn_context: TurnContext):
        ...
        card_path = os.path.join(os.getcwd(), ADAPTIVECARD2)
        mindbot = os.path.join(os.getcwd(), ADAPTIVECARD2)
        with open(card_path, "rb") as in_file:
            template_json = json.load(in_file)
    ...
    async def _option1_card(self, turn_context: TurnContext):
        TeamsChannelAccount: member = None
        try:
            member = await TeamsInfo.get_member(
                turn_context, turn_context.activity.from_property.id
        ...
        card_path = os.path.join(os.getcwd(), OPTION1_CARD)
        mindbot = os.path.join(os.getcwd(), OPTION1_CARD)
        with open(card_path, "rb") as in_file:
            template_json = json.load(in_file)
    ...
    async def _optionA1(self, turn_context: TurnContext):
    ...
    async def _optionA2(self, turn_context: TurnContext):
        team_members = await self._get_paged_members(turn_context)
    ...
    async def _optionA3(self, turn_context: TurnContext):
        team_members = await self._get_paged_members(turn_context)
    ...
    async def _third_card(self, turn_context: TurnContext):
        TeamsChannelAccount: member = None
        try:
            member = await TeamsInfo.get_member(
                turn_context, turn_context.activity.from_property.id
        ...
        card_path = os.path.join(os.getcwd(), ADAPTIVECARD3)
        mindbot = os.path.join(os.getcwd(), ADAPTIVECARD3)
        with open(card_path, "rb") as in_file:
            template_json = json.load(in_file)
    ...
    async def _send_card(self, turn_context: TurnContext, isUpdate):
        buttons = [
            ...
            #),
            CardAction(type=ActionTypes.message_back, title="Datacenter Cloud", text="option1"),
            #CardAction(type=ActionTypes.message_back, title="Find me in Adaptive Card", text="mention me"),
            #CardAction(type=ActionTypes.message_back, title="Delete card", text="deletecard"),
        ]
        if isUpdate:
    ...
    async def _send_welcome_card(self, turn_context: TurnContext, buttons):
        ...
        card = HeroCard(
            title=f"{botname} Greeting", text=f"Hello {member.name}, How may I help you today? 🤖 Please type {botname} to start the process", buttons=buttons
        )
        await turn_context.send_activity(
            MessageFactory.attachment(CardFactory.hero_card(card))
        )

    async def _user_input(self, turn_context: TurnContext, buttons):
        member = await TeamsInfo.get_member(
                turn_context, turn_context.activity.from_property.id
            )
        ...
        card = HeroCard(
            title="{botname} Greeting", text=f"Hi {member.name}, How may I help you today? 🤖 Please type {botname} to start the process", buttons=buttons
        )
        await turn_context.send_activity(
            MessageFactory.attachment(CardFactory.hero_card(card))
        )
    ...
    async def _get_netbox(self, turn_context: TurnContext):
        TeamsChannelAccount: response = None
        response = f'The output is: {requests.get("http://159.89.34.192:8000/api/").text}'
    ...
    async def _return_netbox(self, turn_context: TurnContext):
        team_members = await self._get_paged_members(turn_context)
